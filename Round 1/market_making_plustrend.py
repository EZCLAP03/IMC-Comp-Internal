...
class Trader:
    ...
    def squid_ink(state: Status) -> List[Order]:
        ...
        max_buy_allowed = 50 - (state.position if hasattr(state, 'position') else 0)
        max_sell_allowed = 50 + (state.position if hasattr(state, 'position') else 0)

        bid_aggression = 1 if spread > 2 else 0
        ask_aggression = 1 if spread > 2 else 0

        if hasattr(state, 'history') and len(state.history) >= 20:
            recent_prices = state.history[:20]  
            sma = statistics.mean(recent_prices)
            sd = statistics.stdev(recent_prices, sma)
            lower_band = sma - 2 * sd
            upper_band = sma + 2 * sd
        ...
        if state.bids:
            ...
            bid_price_to_quote = min(bid_price + bid_aggression, state.best_ask - 1) if bid_aggression > 0 else bid_price

            if buy_qty > 0:
                orders.append(Order(state.product, bid_price_to_quote, buy_qty))

        if state.asks:
            best_ask_level = min(state.asks, key=lambda x: x[0])  
            ask_price, ask_vol = best_ask_level
            sell_qty = min(
            ...
            ask_price_to_quote = max(ask_price - ask_aggression, state.best_bid + 1) if ask_aggression > 0 else ask_price

            if sell_qty > 0:
                orders.append(Order(state.product, ask_price_to_quote, -sell_qty))
